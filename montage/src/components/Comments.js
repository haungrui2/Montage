    ...
    return(
      <div id = "movie_comment">
        <div id = "comment_input" style = {{display: userId.isLogin ? "block" : "none"}}>
          ...
          <div id= "rate_button">
              <button className="plus_btn" onClick={() => {if(commentRate < 10)
                setCommentRate(commentRate+1)}}>
                +
              </button>
              <button className="minus_btn" onClick={() => {if(commentRate > 0)
                setCommentRate(commentRate-1)}}>
                -
              </button>
          </div>
          <button id="rate">rate:{commentRate}/10 ★</button>
          <button id = "comment_button" onClick={() => {if(isAbleToComment)
            dispatch(addCommentAsync({
                  commentsContent: {userId: userId.uid,
